...
def get_iran_now():
    """تاریخ و ساعت کامل فعلی به وقت ایران (datetime object)"""
...
def describe_strategy(rules):
    ...
    lines = []
    for r in R_LEVELS[:-1]:  # R10 همیشه خروج کامل است، جزو سوالات نیست
        key = str(r)
        rule = rules.get(key)
    ...
    lines.append("🔸 R10 → خروج کامل (همیشه ثابت)")
    return "\n".join(lines)
