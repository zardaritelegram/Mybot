...
def run_custom_simulation(candles, entry_price, sl_price, direction, strategy_rules):
    """
    strategy_rules: dict مثل {"0.5": None, "1": "trail_entry", "1.5": None, "2": 1.15700, ...}
    مقدار None یعنی SL تغییر نمی‌کند در آن سطح
    مقدار "trail_entry" یعنی SL = نقطه ورود
    مقدار عددی یعنی SL = همان عدد دستی کاربر
...
async def handle_backtest(query, user_id):
    ...
    if data == "fin_backtest":
        ...
        if is_yes:
            ...
            await query.edit_message_text(
                f"🎯 در R{r_mult}، حد ضرر جدید را وارد کنید:\n\n"
                f"می‌توانید بنویسید `entry` برای ریسک‌فری (= نقطه ورود)\n"
                f"یا یک عدد دقیق مثل `1.15700`",
                parse_mode="Markdown"
            )
        else:
...
async def handle_backtest_message(user_id, text, update):
    ...
    # ─── دریافت مقدار SL برای یک سطح R خاص ───
    if state and state.startswith("bt_strat_slval_"):
        ...
        if text.lower() == "entry":
            rules[r_mult] = "trail_entry"
        else:
            try:
                rules[r_mult] = float(text.replace(",", ""))
            except:
                await update.message.reply_text("❌ لطفاً یک عدد معتبر یا کلمه `entry` وارد کنید:", parse_mode="Markdown")
                return True
    ...
    # ─── حد ضرر اولیه ───
        ...
        try:
            ...
            set_state(user_id, "bt_timeframe",
                      f"{mode}|{strategy_name}|{rules_str}|{symbol}|{direction}|{entry_date}|{entry_time}|{entry_price}|{sl_price}")
