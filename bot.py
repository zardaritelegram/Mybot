...
# ║  + خط fin_backtest/bt_ رو از handle_financial ROUTER پاک کن ║
# ║  + state های bt_ رو از message_handler پاک کن               ║
# ╚══════════════════════════════════════════════════════════════╝

async def get_historical_candles(symbol, entry_date, interval="1d"):
    """دریافت کندل‌های تاریخی با تایم‌فریم انتخابی کاربر (1m/15m/1h/1d)"""
    try:
        days_ago = (datetime.now() - datetime.strptime(entry_date, "%Y-%m-%d")).days

        limits = {"1m": 7, "15m": 60, "1h": 730, "1d": 3650}
        max_days = limits.get(interval, 3650)
        if days_ago > max_days:
            return None, f"⚠️ تایم‌فریم انتخابی فقط تا {max_days} روز گذشته را پشتیبانی می‌کند.\nتاریخ شما {days_ago} روز پیش است.\nتایم‌فریم بزرگ‌تری انتخاب کنید."

        range_map = {
            "1m": f"{min(days_ago + 1, 7)}d",
            "15m": f"{min(days_ago + 1, 60)}d",
            "1h": f"{min(days_ago + 5, 730)}d",
            "1d": f"{min(days_ago + 5, 3650)}d",
        }
        range_param = range_map.get(interval, "1y")
        ...
        async with httpx.AsyncClient(timeout=15, headers={"User-Agent": "Mozilla/5.0"}) as client:
            ...
            candles = []
            entry_ts = datetime.strptime(entry_date, "%Y-%m-%d").timestamp()
...
def run_position_simulation(candles, entry_price, sl_price, direction):
    """
    شبیه‌سازی مدیریت پوزیشن:
    R1 -> ریسک‌فری | R2 تا R4.5 -> تریل پله‌ای 0.5R | R5 -> خروج کامل
    """
    ...
    current_sl = sl_price
    max_r_reached = 0
    trail_steps = [1, 2, 2.5, 3, 3.5, 4, 4.5, 5]
    next_step_idx = 0
    ...
    for candle in candles:
        ...
        if hit_sl:
            ...
            return {
                "exit_price": current_sl, "exit_time": candle["time"],
                "result_r": round(exit_r, 2), "max_r": max_r_reached,
                "status": "closed"
            }

        reached = high if is_long else low
        while next_step_idx < len(trail_steps):
            target_level = level(trail_steps[next_step_idx])
            condition = (reached >= target_level) if is_long else (reached <= target_level)
            if condition:
                max_r_reached = trail_steps[next_step_idx]
                if trail_steps[next_step_idx] == 1:
                    current_sl = entry_price
                elif trail_steps[next_step_idx] >= 2:
                    prev_step = trail_steps[next_step_idx - 1] if next_step_idx > 0 else 1
                    current_sl = level(prev_step)

                if trail_steps[next_step_idx] == 5:
                    return {
                        "exit_price": level(5), "exit_time": candle["time"],
                        "result_r": 5.0, "max_r": 5.0, "status": "closed"
                    }
                next_step_idx += 1
            else:
                break
    ...
    return {
        ...
        "result_r": round(open_r, 2), "max_r": max_r_reached,
        "status": "open"
    }
...
async def handle_backtest(query, user_id):
    ...
    if data == "fin_backtest":
        await query.edit_message_text(
            ...
            "این ابزار با گرفتن نقطه ورود و حد ضرر، مسیر قیمت رو شبیه‌سازی می‌کند:\n\n"
            "R1 → ریسک‌فری\n"
            "R2 تا R4.5 → تریل پله‌ای (هر 0.5R)\n"
            "R5 → خروج کامل\n",
            reply_markup=backtest_menu()
        )

    elif data == "bt_new":
        set_state(user_id, "bt_symbol")
        await query.edit_message_text(
            "📊 بک‌تست جدید\n\nنماد را وارد کنید:\nمثال: `EURUSD` `GOLD` `USDCAD`",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="fin_backtest")]])
        )

    elif data in ["bt_dir_long", "bt_dir_short"]:
        direction = "long" if data == "bt_dir_long" else "short"
        state, sdata = get_state(user_id)
        if state == "bt_direction" and sdata:
            symbol = sdata
            set_state(user_id, "bt_entry_date", f"{symbol}|{direction}")
            await query.edit_message_text(
...
async def handle_backtest_message(user_id, text, update):
    ...
    if state == "bt_symbol":
        symbol = text.upper()
        set_state(user_id, "bt_direction", symbol)
        await update.message.reply_text(
        ...
        try:
            ...
            parts = data.split("|")
            symbol, direction, entry_date = parts[0], parts[1], parts[2]
            set_state(user_id, "bt_sl_price", f"{symbol}|{direction}|{entry_date}|{entry_price}")
        ...
        try:
            ...
            parts = data.split("|")
            symbol, direction, entry_date, entry_price = parts[0], parts[1], parts[2], float(parts[3])
            ...
            set_state(user_id, "bt_timeframe", f"{symbol}|{direction}|{entry_date}|{entry_price}|{sl_price}")
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("1 روز (بدون محدودیت)", callback_data="bt_tf_1d")],
                ])
            )
        except:
            await update.message.reply_text("❌ لطفاً یک عدد وارد کنید:")
...
async def run_backtest_final(user_id, query):
    ...
    parts = sdata.split("|")
    symbol, direction, entry_date, entry_price, sl_price, timeframe = (
        parts[0], parts[1], parts[2], float(parts[3]), float(parts[4]), parts[5]
    )

    candles, error = await get_historical_candles(symbol, entry_date, timeframe)
    if error:
        await query.message.reply_text(error, reply_markup=backtest_menu())
        clear_state(user_id)
        return

    result = run_position_simulation(candles, entry_price, sl_price, direction)
    R = abs(entry_price - sl_price)

    status_text = "🟢 بسته شده" if result["status"] == "closed" else "🟡 هنوز باز (تا آخرین داده)"
    exit_line = (f"خروج: {result['exit_price']:.5f}\nتاریخ خروج: {result['exit_time'].strftime('%Y-%m-%d %H:%M')}"
    ...
    await query.message.reply_text(text, parse_mode="Markdown", reply_markup=backtest_menu())
    clear_state(user_id)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("menu_financial") or data.startswith("fin_") or data.startswith("wl_") or data.startswith("alarm_") or data.startswith("back_financial"):
        if data.startswith("fin_backtest") or data.startswith("bt_"):
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif state in ["bt_symbol", "bt_entry_date", "bt_entry_price", "bt_sl_price"]:
